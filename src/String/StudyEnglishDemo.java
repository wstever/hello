...
public class StudyEnglishDemo {
	...
	static String[] a = new String[] { "", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };// 定义数字1-9
	...
	static String[] c = new String[] { "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen",
			"seventeen", "eighteen", "nineteen" };// 定义数字整十
				...
				case 2:// 长度为7-9位
					if (parse(n.substring(m, m + 3)).equals("") && parse(n.substring(m + 3, m + 6)).equals("")) {// 后六位全为零输出
						System.out.println(parse(n.substring(0, m)) + " million");
							...
							&& !parse(n.substring(m + 3, m + 6)).equals("")) {// 中间三位为0，后三位不为0
						System.out.println(parse(n.substring(0, m)) + " million " + parse(n.substring(m + 3, m + 6)));
					} else if (!parse(n.substring(m, m + 3)).equals("")
							&& parse(n.substring(m + 3, m + 6)).equals("")) {// 中间不为零，后三位为0
